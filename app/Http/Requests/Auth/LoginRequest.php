...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        if (! Auth::attempt(['username' => $this->input('username'), 'password' => $this->input('password')], $this->boolean('remember'))) {
            ...
            throw ValidationException::withMessages([
                'username' => trans('auth.failed'), // 🔄 Changed from 'email' to 'username'
            ]);
        }
    ...
    {
        return Str::transliterate(Str::lower($this->input('username')).'|'.$this->ip()); // 🔄 Changed from 'email' to 'username'
    }
}
